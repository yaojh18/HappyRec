...
class DataReader(object):
    ...
    def read_train(self, filename: str, formatters: dict) -> dict:
        ...
        DEFAULT_LOGGER.info("train df length = {}".format(len(df)))
        self.train_data = df2dict(df, formatters=formatters)
        DEFAULT_LOGGER.debug("train data keys {}:{}".format(len(self.train_data), list(self.train_data.keys())))
        return self.train_data

    def read_validation(self, filename: str, formatters: dict) -> dict:
        DEFAULT_LOGGER.debug("read {}...".format(filename))
        df = read_df(dirname=self.dataset_dir, filename=filename)
        DEFAULT_LOGGER.info("validation df length = {}".format(len(df)))
        self.val_data = df2dict(df, formatters=formatters)
        DEFAULT_LOGGER.debug("validation data keys {}:{}".format(len(self.val_data), list(self.val_data.keys())))
        return self.val_data

    def read_test(self, filename: str, formatters: dict) -> dict:
        DEFAULT_LOGGER.debug("read {}...".format(filename))
        df = read_df(dirname=self.dataset_dir, filename=filename)
        DEFAULT_LOGGER.info("test df length = {}".format(len(df)))
        self.test_data = df2dict(df, formatters=formatters)
        DEFAULT_LOGGER.debug("test data keys {}:{}".format(len(self.test_data), list(self.test_data.keys())))
