...
from happyrec.utilities.dataset import renumber_ids, copy_ui_features, leave_out_csv, random_sample_eval_iids
...
def format_amazon_inter(inter_df, out_name, label01=False):
    # 给uid编号，从1开始
    inter_df, user_df, uid_dict = renumber_ids(inter_df, old_column='user_id', new_column=UID)
    user_df.to_csv(os.path.join(AMAZON2014_DIR, '{}.uid_dict.csv'.format(out_name)), sep='\t', index=False)
    user_df[[UID]].to_csv(os.path.join(AMAZON2014_DIR, '{}.users.csv'.format(out_name)), sep='\t', index=False)

    # 给iid编号，从1开始
    inter_df, item_df, iid_dict = renumber_ids(inter_df, old_column='item_id', new_column=IID)
    item_df.to_csv(os.path.join(AMAZON2014_DIR, '{}.iid_dict.csv'.format(out_name)), sep='\t', index=False)
    item_df[[IID]].to_csv(os.path.join(AMAZON2014_DIR, '{}.items.csv'.format(out_name)), sep='\t', index=False)
...
def format_rating_only(category='', in_csv='', label01=False):
    ...
    inter_df = pd.read_csv(in_csv, header=None, names=['user_id', 'item_id', LABEL, TIME])
...
def format_5core(category='', in_json='', label01=False):
    ...
    inter_df['user_id'] = [r['reviewerID'] for r in records]
    inter_df['item_id'] = [r['asin'] for r in records]
